...
export default function NonIdealStateExample(IExampleProps: IExampleProps) {
    ...
    const toggleShowAction = handleBooleanChange((showAction: boolean) => setState({ 
        ...state,
        showAction: showAction
    }));

    const toggleShowDescription = handleBooleanChange((showDescription: boolean) => setState({  
        ...state,
        showDescription: showDescription
    }));

    const toggleShowTitle = handleBooleanChange((showTitle: boolean) => setState({ 
        ...state,
        showTitle: showTitle 
    }));

    const handleIconNameChange = (icon?: IconName) => setState({  
        ...state,
        icon: icon 
    });

    const handleLayoutChange = (layout: Layout) => setState({ 
        ...state,
        layout: layout
     });

    const handleSizeChange = (size: Size) => setState({ 
        ...state,
        iconSize: sizeToNonIdealStateIconSize[size]
    });

    const handleVisualKindChange = (visual: VisualKind) => setState({ 
        ...state,
        visual: visual 
    });
